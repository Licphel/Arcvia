...
static void spread_liquid_(obs<chunk>& chunk_, obs<chunk>& chunk0, obs<chunk>& chunk1, obs<chunk>& chunk2,
                           obs<chunk>& chunk3, int x, int y) {
    pos2i p_0 = pos2i(x, y);
    liquid_stack qstack = chunk_->find_liquid_stack(p_0);
    ...
    if (chunk2 || soy != ARC_CHUNK_SIZE - 1) {
        if (soy == ARC_CHUNK_SIZE - 1) {
            bd = chunk2->find_block(p_d);
            chunkd = chunk2;
            qstackd = chunkd->find_liquid_stack(p_d);
        } else {
            bd = chunk_->find_block(p_d);
            chunkd = chunk_;
            qstackd = chunkd->find_liquid_stack(p_d);
        }
        ...
        if (!bd->shape.solid) {
            if (ld != liquid_void && ld != type) {
                type->on_touch(chunk_->dim, p_0, qstack, p_d, qstackd);
            } else if (ad < liquid_stack::max_amount) {
                int ext = std::min(liquid_stack::max_amount - ad, a);
                a -= ext;
                chunk_->set_liquid_stack(liquid_stack(type, a), p_0);
                chunkd->set_liquid_stack(liquid_stack(type, ext + ad), p_d);
                if (ext > a / 2.0) return;
            }
        }
    }
    ...
    if (sox == 0) {
        if (!chunk0) return;
        blockl = chunk0->find_block(p_l);
        blockr = chunk_->find_block(p_r);
        chunkl = chunk0;
        chunkr = chunk_;
        qstackr = chunk_->find_liquid_stack(p_r);
        qstackl = chunkl->find_liquid_stack(p_l);
    } else if (sox == 15) {
        if (!chunk1) return;
        blockl = chunk_->find_block(p_l);
        blockr = chunk1->find_block(p_r);
        chunkl = chunk_;
        chunkr = chunk1;
        qstackr = chunkr->find_liquid_stack(p_r);
        qstackl = chunk_->find_liquid_stack(p_l);
    } else {
        blockl = chunk_->find_block(p_l);
        blockr = chunk_->find_block(p_r);
        chunkl = chunk_;
    ...
    if (spread_leftward && !blockl->shape.solid) {
        if (liquidl != liquid_void && liquidl != type) {
            type->on_touch(chunk_->dim, p_0, qstack, p_l, qstackl);
        } else {
            int ext = std::min(liquid_stack::max_amount - amountl,
                               std::min(static_cast<int>(std::ceil((a - amountl) / 2.0)), a));
            a -= ext;
            chunk_->set_liquid_stack(liquid_stack(type, a), p_0);
            chunkl->set_liquid_stack(liquid_stack(type, ext + amountl), p_l);
        }
    } else if (!blockr->shape.solid) {
        if (liquidr != liquid_void && liquidr != type) {
            type->on_touch(chunk_->dim, p_0, qstack, p_r, qstackr);
        } else {
            int ext = std::min(liquid_stack::max_amount - amountr,
                               std::min(static_cast<int>(std::ceil((a - amountr) / 2.0)), a));
            a -= ext;
            chunk_->set_liquid_stack(liquid_stack(type, a), p_0);
            chunkr->set_liquid_stack(liquid_stack(type, ext + amountr), p_r);
        }
    }
}
