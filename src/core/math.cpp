...
double vec2::length() const { return std::sqrt(length_powered()); }
double vec2::length_powered() const { return x * x + y * y; }

vec2 vec2::from(double len, double rad) { return vec2(std::cos(rad) * len, std::sin(rad) * len); }
...
double vec2::dot(const vec2& v1, const vec2& v2) { return v1.x * v2.x + v1.y * v2.y; }
...
double vec3::length() const { return std::sqrt(length_powered()); }

double vec3::length_powered() const { return x * x + y * y + z * z; }
...
float transform::det() const { return m00 * m11 - m01 * m10; }
...
void transform::apply(float& x, float& y) const {
    double xx = x, yy = y;
    x = m00 * xx + m01 * yy + m02;
    y = m10 * xx + m11 * yy + m12;
}
...
quad quad::center(double x, double y, double w, double h) { return quad(x - w * 0.5, y - h * 0.5, w, h); }

quad quad::corner(double x, double y, double w, double h) { return quad(x, y, w, h); }
...
bool quad::intersect(const quad& q1, const quad& q2) { return intersection_of(q1, q2).area() > 0.0; }
...
double quad::center_x() const { return x + width * 0.5; }
double quad::center_y() const { return y + height * 0.5; }
double quad::corner_x() const { return x; }
